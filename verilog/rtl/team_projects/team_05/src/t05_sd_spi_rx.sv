module t05_sd_spi_rx #(
    parameter WIDTH = 40,                    // Number of bits to receive
    parameter CLK_DIV = 4                   // Clock divider (system_clk / CLK_DIV = spi_clk)
) (
    input  logic                clk,        // System clock
    input  logic                rst_n,      // Active low reset
    input  logic                start,      // Start reception (pulse)
    output logic [WIDTH-1:0]    data_out,   // Received data
    input  logic                sdi,        // Serial data in (MISO)
    output logic                sclk,       // SPI clock
    output logic                cs_n,       // Chip select (active low)
    output logic                busy,       // Reception in progress
    output logic                done        // Reception complete (pulse)
);
    ...
    // Combinational logic for shift register (now for receiving data)
    always_comb begin
        shift_reg_d = shift_reg_q;
        
        if (state_q == IDLE && start) begin
            shift_reg_d = '0;  // Clear register for new reception
        end else if (state_q == ACTIVE && spi_clk_en) begin
            // Sample incoming data on the appropriate clock edge
            if (spi_clk_q) begin
                if (WIDTH > 1) begin
                    shift_reg_d = {shift_reg_q[WIDTH-2:0], sdi};  // Shift left and sample SDI (MSB first)
                end else begin
                    shift_reg_d = sdi;  // Special case for WIDTH=1
                end
            end
        end
    end
    ...
    assign data_out = shift_reg_q;  // Output the received data
    assign busy = !cs_n;
